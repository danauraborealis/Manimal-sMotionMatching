...
2. Close the game and extract the contents of `Manimal-MotionMatching-0.1.1.zip` into the SPT game folder, preserving its `BepInEx/plugins/Manimal-MotionMatching` path.
...
4. After a raid, look for its diagnostic ZIP in `BepInEx/LogOutput/Manimal-MotionMatching/Reports` inside the SPT folder. Send that ZIP with your feedback so the movement and reaction events can be compared with what you saw.
